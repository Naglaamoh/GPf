...
export default function Read() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/acceptedCases/` + id)
  ...
  return (
    <div className="d-flex w-100 justify-content-center align-items-center vh-100 " >
      <div className='read  w-50 rounded  border shadow px-5 pt-5 pb-5 '>
          ...
          {/* <div className="mb-2">
            <strong>Appointment : {data.appointment}</strong>
          </div> */}

          <div className="mb-2">
            <strong>Phone Number : {data.number}</strong>
          </div>
          ...
          <Link to="/cases" className='btn btn-primary mt-4'>Back</Link>
      </div>
    </div>
  )
}
